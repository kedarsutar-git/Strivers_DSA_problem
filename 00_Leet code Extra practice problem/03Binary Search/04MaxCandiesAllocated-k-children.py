...
Output: 5
Explanation: We can divide candies[1] into 2 piles of size 5 and 3, and candies[2] into 2 piles of size 5 and 1. We now have five piles 
of candies of sizes 5, 5, 3, 5, and 1. We can allocate the 3 piles of size 5 to 3 children. It can be proven that each child cannot receive 
more than 5 candies.
...
class Solution:
    def MaxCandies(self,nums:list[int],k:int):
        start = 1
        end = max(nums)
        ans = -1
...
nums = [5,8,6]    
object = Solution()
print(object.MaxCandies(nums,3))
